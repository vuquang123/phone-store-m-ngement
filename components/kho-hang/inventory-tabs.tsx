    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <Card className="p-4 bg-gradient-to-br from-emerald-50 to-blue-50 border-emerald-100 shadow-sm">
        <div className="flex items-start gap-3">
          <div className="bg-card p-2 rounded-lg shadow-sm">📲</div>
          <div className="flex-1">
            <div className="text-sm font-bold text-emerald-800 mb-2">Sản phẩm hệ thống</div>
            <div className="grid grid-cols-4 gap-2">
              <button 
                onClick={() => setActiveTab("san-pham")}
                className="flex flex-col items-center p-2 bg-card/60 rounded-lg border border-emerald-100 hover:bg-card transition-colors"
              >
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button 
                ...
                className="flex flex-col items-center p-2 bg-card/60 rounded-lg border border-rose-100 hover:bg-card transition-colors"
              >
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      <TabsList className="grid w-full grid-cols-5 max-w-3xl bg-muted/50 p-1 rounded-xl h-11">
        <TabsTrigger value="san-pham" className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-emerald-700 font-medium">Sản phẩm</TabsTrigger>
        <TabsTrigger value="phu-kien" className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-emerald-700 font-medium">Phụ kiện</TabsTrigger>
        <TabsTrigger value="dang-cnc" className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-emerald-700 font-medium">CNC</TabsTrigger>
        <TabsTrigger value="bao-hanh" className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-emerald-700 font-medium">Bảo hành</TabsTrigger>
        <TabsTrigger value="giao-doi-tac" className="rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-emerald-700 font-medium">Đối tác</TabsTrigger>
      </TabsList>
    </Tabs>
  )
}
